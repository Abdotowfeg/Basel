...
class AccountAssetCategory(models.Model):
    ...
    open_asset = fields.Boolean(string='Auto-confirm Assets', default=True,
    ...
    account_revaluation_id = fields.Many2one('account.account', string="Revaluation Account", required=True)
    account_enhancement_id = fields.Many2one('account.account', string="Enhancement Account", required=True)
    
    @api.onchange('method_time')
    def onch_method_time(self):
...
class AccountAssetAsset(models.Model):
    ...
    revaluation_count = fields.Integer(compute='_revaluation_count',
    ...
    depreciation_start_date = fields.Date(string='Depreciation Start Date', required=True, readonly=True,
                       states={'draft': [('readonly', False)]}, 
                       default=fields.Date.context_today)
    state = fields.Selection(
        [('draft', 'Draft'), ('open', 'Running'), ('close', 'Close'), ('depreciated', 'Depreciated'),
    ...
    value_residual = fields.Float(compute='_amount_residual', method=True,
                                  digits=0, string='Residual Value', store=True)

    depreciation_accumulated = fields.Float(compute='get_depreciation_accumulated', string='Depreciation Accumulated')
    ...
    type = fields.Selection([('sale', 'Sale: Revenue Recognition'),
                             ('purchase', 'Purchase: Asset')], required=True,
                            index=True, default='purchase')
    seq_name = fields.Char('seq name', readonly=True, copy=False, required=True, default='New')
    hide_button = fields.Boolean()

    enhancement_ids = fields.One2many('account.asset.enhancement', 'asset_id', string="Asset Enhancements")
    # related fields
    method = fields.Selection(
        [('linear', 'Linear'), ('degressive', 'Degressive')],
        string='Computation Method', required=True,
    ...
    def check_open_asset(self):
        ...
        self.journal_id = self.category_id.journal_id.id
        self.account_depreciation_expense_id = self.category_id.account_depreciation_expense_id.id
        self.account_revaluation_id = self.category_id.account_revaluation_id.id
        self.account_enhancement_id = self.category_id.account_enhancement_id.id
        self.account_asset_id = self.category_id.account_asset_id.id
        self.type = self.category_id.type
        self.account_depreciation_id = self.category_id.account_depreciation_id.id
        self.open_asset = self.category_id.open_asset
        self.group_entries = self.category_id.group_entries
        self.account_analytic_id = self.category_id.account_analytic_id.id



    def get_depreciation_accumulated(self):
        for rec in self:
            rec.depreciation_accumulated = sum(rec.depreciation_line_ids.filtered(lambda l: l.move_check).mapped('amount'))
    ...
    def compute_depreciation_board(self):
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata:
                ...
                if self.method_period >= 12:
                    asset_date = datetime.strptime(
                        str(self.depreciation_start_date),DF).date()
                else:
                    asset_date = datetime.strptime(str(self.depreciation_start_date),
                                                   DF).date()
            ...
            revaluation_ids = self.env['account.asset.revaluation'].search([('asset_id', '=', self.id)])
            revaluation_year = False
            enhancement_ids = self.env['account.asset.enhancement'].search([('asset_id', '=', self.id)])
            enhancement_year = False

            for x in range(len(posted_depreciation_line_ids),
                           undone_dotation_number):
                sequence = x + 1
                year_period = depreciation_date + relativedelta(years=1)

                if revaluation_ids:
                    if sequence == 1:
                        revaluation_year = revaluation_ids.filtered(
                            lambda r: r.revaluation_date.date() >= self.date and r.revaluation_date.date() < year_period)
                    if sequence > 1:
                        revaluation_year = revaluation_ids.filtered(
                            lambda r: r.revaluation_date.date() >= depreciation_date and r.revaluation_date.date() < year_period)
                    if sequence == self.method_number:
                        revaluation_year = revaluation_ids.filtered(
                            lambda r: r.revaluation_date.date() >= depreciation_date)

                    if revaluation_year:
                        revaluation_year = revaluation_year[-1]

                if enhancement_ids:

                    if sequence == 1:
                        enhancement_year = enhancement_ids.filtered(
                            lambda r: r.enhancement_date.date() >= self.date and r.enhancement_date.date() < year_period)
                    if sequence > 1:
                        enhancement_year = enhancement_ids.filtered(
                            lambda r: r.enhancement_date.date() >= depreciation_date and r.enhancement_date.date() < year_period)
                    if sequence == self.method_number:
                        enhancement_year = enhancement_ids.filtered(
                            lambda r: r.enhancement_date.date() >= depreciation_date)
                    # enhancement_year = enhancement_ids.filtered(
                    #     lambda r: r.enhancement_date.year == depreciation_date.year)
                    if enhancement_year:
                        enhancement_year = enhancement_year[-1]

                year_posted = posted_depreciation_line_ids.filtered(
                        lambda r: r.depreciation_date.year == depreciation_date.year)

                if revaluation_year and not year_posted:
                    amount_to_depr = revaluation_year.revalued_value
                    undone_dotation_number = self.method_number - x + len(posted_depreciation_line_ids)
                    residual_amount = amount_to_depr - (amount_to_depr / undone_dotation_number - len(posted_depreciation_line_ids))

                if enhancement_year and not year_posted:

                    if not revaluation_year:
                        undone_dotation_number = self.method_number - x + len(posted_depreciation_line_ids)
                        amount_to_depr = enhancement_year.revalued_value
                        residual_amount = amount_to_depr - (amount_to_depr / (undone_dotation_number - len(posted_depreciation_line_ids)))
                    if revaluation_year:
                        if enhancement_year.enhancement_date > revaluation_year.revaluation_date:
                            amount_to_depr = enhancement_year.revalued_value
                            residual_amount = amount_to_depr - (amount_to_depr / undone_dotation_number)
                ...
                if float_is_zero(amount,
                    ...
                    continue
                if not revaluation_year and not enhancement_year:
                    residual_amount -= amount

                # if not enhancement_year:
                #     residual_amount -= amount
    ...
    def _get_disposal_moves(self):
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                # asset.category_id.write(
                #     {
                #      }
                # )
                tracked_fields = self.env['account.asset.asset'].fields_get(
                    ['method_number', 'method_end'])
                changes, tracking_value_ids = asset.category_id._message_track(
    ...
    def set_to_close(self):
        self.write({'state': 'sold_disposed'})
    ...
    @api.constrains('value_residual')
    def set_to_depreciated(self):
        for rec in self:
            if rec.value_residual == 0.0:
                self.write({'state': 'depreciated'})
    ...
    def _entry_count(self):
        ...
        res = self.env['account.move'].search_count(['|',('id', 'in', move_ids),('ref', '=', self.name)])
        asset.entry_count = res or 0

    def _revaluation_count(self):
        for asset in self:
            res = self.env['account.asset.revaluation'].search_count(
                [('asset_id', '=', asset.id)])
            asset.revaluation_count = res or 0
    ...
    def copy_data(self, default=None):
        ...
        # schedular to run asset after 1 month
    ...
    def _compute_entries(self, date, group_entries=False):
        depreciation_ids = self.env['account.asset.depreciation.line'].search([
            ...
            ('move_check', '=', False)])
        print(date,"xxxxxxxxxxxxxxxxxxxxxxxxxxxx",self.ids," _compute_entries xxxxxxxxxxxxxxxxxxxxxxxxxxx")
        # print("date >>>>> >>>>>>>>>>>>>>>>",date )
        if group_entries:
    ...
    def create(self, vals):
        if vals.get('seq_name', 'New') == 'New':
            vals['seq_name'] = self.env['ir.sequence'].next_by_code('account.asset.sequence') or 'New'
        # values = {
        #     'name': 'asset_sequence',
        #     'code': 'asset_sequence_id',
        #     'implementation': 'no_gap',
        #     'prefix': self.date,
        #     'suffix': '',
        #     'padding': 0,
        # }
        # self.seq_name = self.env['ir.sequence'].create(values)
        # print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@',self.seq_name)
        # vals['seq_name']= seq.name
    ...
    def open_entries(self):
        ...
        return {
            ...
            'view_id': False,
            'type': 'ir.actions.act_window',
            'domain': ['|',('id', 'in', move_ids),('ref', '=', self.name)],
        }

    def open_revaluation(self):
        return {
            'name': _('Asset Revaluation'),
            'view_mode': 'tree,form',
    ...
    def show_asset_revaluation(self):
        return {
            'name': _('Asset Revaluation'),
            'view_mode': 'form',
            'res_model': 'account.asset.revaluation',
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            move_line_1 = {
                ...
                'analytic_account_id': line.asset_id.account_analytic_id.id if category_id.type == 'sale' else False,
            ...
            move_line_2 = {
                ...
                'journal_id': line.asset_id.journal_id.id,
                'partner_id': partner.id,
                'analytic_account_id': line.asset_id.account_analytic_id.id if category_id.type == 'purchase' else False,
                'currency_id': company_currency != current_currency and current_currency.id or False,
    ...
    def create_grouped_move(self, post_move=True):
        ...
        move_line_1 = {
            'name': name,
            'account_id': self[0].asset_id.account_depreciation_id.id,
            'debit': 0.0,
            'credit': amount,
            'journal_id': self[0].asset_id.journal_id.id,
            'analytic_account_id': self[0].asset_id.account_analytic_id.id if category_id.type == 'sale' else False,
        }
        move_line_2 = {
            'name': name,
            'account_id': self[0].asset_id.account_depreciation_expense_id.id,
            'credit': 0.0,
            'debit': amount,
            'journal_id': self[0].asset_id.journal_id.id,
            'analytic_account_id': self[0].asset_id.account_analytic_id.id if category_id.type == 'purchase' else False,
        }
        move_vals = {
...
class AccountAccount(models.Model):
    ...
    automate_asset = fields.Boolean(string="Automate Asset")
    asset_model = fields.Many2one('account.asset.category', string="Asset Model")
    asset_account = fields.Boolean(string="Asset Account", compute='check_assets_accounts')

    def check_assets_accounts(self):
        types = self.env.ref('account.data_account_type_fixed_assets') + self.env.ref(
            'account.data_account_type_non_current_assets')
        if self.user_type_id in types:
            self.asset_account = True
        else:
            self.asset_account = False
