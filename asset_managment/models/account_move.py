...
class AccountMove(models.Model):
    ...
    asset_depreciation_ids = fields.One2many('account.asset.depreciation.line',
                                             ...
                                             ondelete="restrict")
    asset_count = fields.Integer(compute="get_asset_count")

    def get_asset_count(self):
        invoice_asset_ids = self.invoice_line_ids.mapped('asset_category_id')
        asset_ids = self.env['account.asset.asset'].search([('category_id', 'in', invoice_asset_ids.ids)])
        # asset_ids = set(asset_ids.mapped('category_id'))
        self.asset_count = len(asset_ids.ids)

    def open_assets(self):
        action = self.env.ref('asset_managment.action_account_asset_asset_form').read()[0]
        invoice_asset_ids = self.invoice_line_ids.mapped('asset_category_id')
        asset_ids = self.env['account.asset.asset'].search([('category_id', 'in', invoice_asset_ids.ids)])
        # asset_ids = [id for id in set(asset_ids.mapped('category_id').ids)]
...
class AccountInvoiceLine(models.Model):
    ...
    def asset_create(self):
        for record in self:
            if record.account_id.asset_account:
                vals = {
                    'name': record.product_id.name,
                    'code': record.move_id.name or False,
                    'category_id': record.account_id.asset_model.id,
                    ...
                    'journal_id' : record.account_id.asset_model.journal_id.id,
                    'account_depreciation_expense_id' : record.account_id.asset_model.account_depreciation_expense_id.id,
                    'method' : record.account_id.asset_model.method,
                    'method_time' : record.account_id.asset_model.method_time,
                    'prorata' : record.account_id.asset_model.prorata,
                    'method_number' : record.account_id.asset_model.method_number,
                    'method_period' : record.account_id.asset_model.method_period,
                    'method_end' : record.account_id.asset_model.method_end,
                    'account_revaluation_id': record.account_id.asset_model.account_revaluation_id.id,
                    'account_enhancement_id': record.account_id.asset_model.account_enhancement_id.id,
                    'account_asset_id' : record.account_id.asset_model.account_asset_id.id,
                    'type': record.account_id.asset_model.type,
                    'account_depreciation_id' : record.account_id.asset_model.account_depreciation_id.id,
                    'open_asset': record.account_id.asset_model.open_asset,
                    'group_entries': record.account_id.asset_model.group_entries,
                    'account_analytic_id' : record.account_id.asset_model.account_analytic_id.id
                }
                ...
                if record.account_id.automate_asset:
                    asset.validate()
        return True
